...
SYNTAX:
  ./$(basename "$0") <input_file> [flux_variant] [bpd] [extrap_mode] [--no-ms]
  ./$(basename "$0") -h | --help
...
POSITIONAL ARGUMENTS:
  ...
  extrap_mode   Extrapolation mode for SACS: 1 = Enabled (default), 0 = Disabled
...
EXAMPLES:
  ./$(basename "$0") input_NEAR_BurialDating MCecc 200 1       # Standard run (Extrapolation ON)
  ./$(basename "$0") input_EAR1_BurialDating CCeval-EAR1 200 0 # Disable SACS Extrapolation
  ./$(basename "$0") inputs/input_NEAR_N14 MCecc 200 1 --no-ms # Fast Run (No MC)
===============================================================================
EOF
}
...
echo "Extrapolation   : ${EXTRAP_MODE}"
...
python3 "${COUNTS_SCRIPT}" "${INPUT_FILE}" "${FLUX_VARIANT}" "${BPD}" "${EXTRAP_MODE}"
